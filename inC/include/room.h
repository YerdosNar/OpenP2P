...
typedef struct {
        ...
        Session         session;        // E2EE key for the host
        uint32_t        fd;
        uint16_t        port;
        bool            is_active;
} Room;
...
/* Returns a pointer to the active room the given ID, or NULL */
Room *room_find_by_id(Room rooms[], uint32_t max_rooms, const char *id);

/* Returns true if an active room with the fiven id already exists. */
