 ...
 * Key exchange : X25519 via crypt_kx_*
 ...
 *      +---------------------------------------+
 *
 * The MAC (ABYTES=16) is appended by libsodium inside
 ...
 * On failure: zeros any derived material and returns false.
 */
bool crypto_do_key_exchange(int32_t fd, Session *s);

/*
 * Encrypt 'msg' (NULL-terminated) with s->tx, prepend a radom
 * nonce and the plaintext length, then send over fd.
 *
 ...
 * NULL-terminated plaintext into *out_buf (caller must free).
 *
 * Returns true on success.
 */
