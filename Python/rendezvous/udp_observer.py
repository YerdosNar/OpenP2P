"""
UDP endpoint for the Rendezvous server.

Listens for probe packets (16-byte nonces) and reports observed source
addresses back to the TCP handlers that are waiting on those nonces.
...
class UDPObserver(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data: bytes, addr: tuple) -> None:
        # Accept only exact-sized probes; anything else is noise/garbage.
        if len(data) != NONCE_SIZE:
            return
        future = self._pending.pop(data, None)
        if future is None:
            # No session waiting for this nonce. Could be a late retry from
            # a session that already succeeded — perfectly normal, ignore.
            return
        if not future.done():
            future.set_result((addr[0], addr[1]))

    def error_received(self, exc: Exception) -> None:
        # UDP errors (e.g., ICMP unreachable) — log and carry on.
        log.debug("UDP error: %s", exc)

    # ---- API used by TCP session handlers ----

    def register_nonce(self, nonce: bytes) -> asyncio.Future:
        """
        Reserve a future for a given nonce. The TCP handler awaits this;
