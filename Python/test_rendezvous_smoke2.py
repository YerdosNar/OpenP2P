...
async def smoke():
    ...
    udp_sock.sendto(nonce, ("127.0.0.1", 8888))
    ...
    if observed["port"] == local_udp_port:
        print(f"UDP observation matched local port: SUCCESS (simple/cone NAT or loopback)")
    else:
        print(f"UDP observation returned different port {observed['port']} vs local {local_udp_port}")
        print("This is still SUCCESS - it means NAT is translating ports, which is normal.")
