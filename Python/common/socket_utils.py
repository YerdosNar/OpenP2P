...
reusing the Rendezvous-facing port as the QUIC-facing port without losing
the NAT mapping.
"""
...
def create_reusable_udp_socket(
    ...
    # SO_REUSEADDR: always set. Cheap, universally available.
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

    # SO_REUSEPORT: only exists on Linux and BSD/macOS. Not on Windows.
    # The attribute check is the right way — don't rely on sys.platform
    # because different Python builds expose different constants.
    if hasattr(socket, "SO_REUSEPORT"):
        try:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
        except OSError:
            # Some kernels expose the constant but refuse the call
            # (older Linux, odd BSD variants). Not fatal — REUSEADDR
            # alone is often sufficient for our use case.
            pass

    # Non-blocking so we can hand this to asyncio later.
...
KEEPALIVE_INTERVAL = 15.0   # seconds — comfortably under typical NAT timeouts
KEEPALIVE_PAYLOAD = b"\x00"  # single byte is enough to refresh the mapping
    ...
    try:
        while True:
            ...
            try:
                ...
                # Socket may be closing; don't spam errors, just exit quietly.
                return
    except asyncio.CancelledError:
        # Normal shutdown path. Re-raise so the caller sees cancellation.
        raise
...
def describe_platform_support() -> str:
    ...
    Handy to print at startup when debugging hole-punch issues on new hardware."""
