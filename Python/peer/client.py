"""
Peer client — Rendezvous side + P2P QUIC bring-up.
"""
...
class PeerClient:
    ...
    async def run(self) -> Optional[PeerHandoff]:
        ...
        # 2. UDP socket kept for the whole P2P lifetime
...
async def _main(rendezvous_host: str, rendezvous_port: int):
    ...
    # Silence noisy aioquic internals unless you really want them; flip to
    # DEBUG if the handshake still fails after these changes.
    ...
    print("\n=== Starting QUIC ===")
    print(f"  UDP socket fd={handoff.udp_socket.fileno()}, "
          f"bound to {handoff.udp_socket.getsockname()}, "
          f"talking to {peer_addr}")
    ...
    print("Waiting for QUIC handshake (up to 10s)...")
    ...
    # Minimal hello-world exchange.
    received = asyncio.Event()

    async def on_data(sid, data, end):
        print(f"<< received on stream {sid}: "
    ...
    greeting = (
        f"Hello from {'host' if handoff.is_host else 'joiner'}!"
    ).encode()
    print(f">> sending on stream {stream_id}: {greeting!r}")
    quic_peer.send_stream(stream_id, greeting, end_stream=False)

    # Wait up to 5s for the other side's hello.
    try:
        await asyncio.wait_for(received.wait(), 5.0)
    except asyncio.TimeoutError:
        print("(did not receive peer's hello within 5s)")

    # Linger briefly so both sides can see each other's messages.
